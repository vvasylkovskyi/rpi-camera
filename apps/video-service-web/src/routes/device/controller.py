...
from shared.clients.aws_iot_client import AwsIoTClient
...
@device_router.get("/register")
def register_device():
    aws_iot_client = AwsIoTClient()
    device_id = f"device-{uuid.uuid4().hex[:8]}"
    device_credentials = aws_iot_client.provision_device_credentials(device_id)
...
def update_device(
    device_id: int, device: Device, session: Session = Depends(get_session)
):
    device_manager = DeviceManager(session)
...
async def get_health_check(_: Request, user_id: str = Depends(get_current_user)):
    ...
    try:
        result: dict = await mqtt_rpc_client.call(
            MQTTTopics.DEVICE_CONTROL.with_device(device_id), event.json()
        )
