...
from shared.models.battery_info_event import (
    BatteryAction,
    BatteryInfoRequestEvent,
    BatteryInfoResponseEvent,
)
...
async def health_check(_: Request):
    ...
    try:
        result: dict = await mqtt_rpc_client.call(
            MQTTTopics.BATTERY_CONTROL.with_device(device_id), event.json()
        )
